...
class Magnatune(rb.Plugin):
		...
		if has_gnome_keyring:
			...
			try:
				id = gnomekeyring.item_create_sync(self.get_keyring(),
						...
						"Magnatune credit card info", {"name": "rb-magnatune-cc-data"},
						string.join (details, '\n'), True)
			except Exception, e:
				print e
	...
	def clear_cc_details(self):
		if has_gnome_keyring:
			...
			try:
				ids = gnomekeyring.find_items_sync (gnomekeyring.ITEM_GENERIC_SECRET, {"name": "rb-magnatune-cc-data"})
				print "keyring ids " + str(ids)
				gnomekeyring.item_delete_sync (self.get_keyring(), ids[0])
			except Exception, e:
				print e
